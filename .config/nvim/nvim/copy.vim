...
let g:LanguageClient_serverCommands = { 'go': ['gopls'], 'rust': ['~/.cargo/bin/rustup', 'run', 'stable', 'rls'], 'javascript': ['/usr/local/bin/javascript-typescript-stdio'], 'javascript.jsx': ['tcp://127.0.0.1:2089'], 'python': ['/usr/local/bin/pyls'], 'reason': ['/usr/bin/reason-language-server'], }
...
let g:ale_linters = {	'go': ['gopls', 'gofmt'],	'jsx': ['eslint']}
...
""" FZF
let g:rg_command = ' rg --column --line-number --no-heading --fixed-strings --ignore-case --no-ignore --hidden --follow --color "always" -g "*.{js,json,md,html,config,py,cpp,c,h,hpp,rs,elm,jsx,toml,go,hs,rb,conf,tf,yml,ts,vim,re}" -g "!{.git,node_modules,vendor,.DS_Store,target,}/*" -g "!*.{bs.js}" '
